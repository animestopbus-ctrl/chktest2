...
# Rate limiter for broadcasts (high traffic)
BROADCAST_LIMITER = asyncio.Semaphore(5)  # 5 concurrent broadcasts

# ---------------------------------------------------
# Broadcast helper function (Async optimized)
# ---------------------------------------------------
...
# ---------------------------------------------------
# /broadcast command (Handles high traffic)
# ---------------------------------------------------
...
async def broadcast_command(bot: Client, message: Message):
    ...
    async for user in users:
        user_id = user.get('id')
        if user_id and not await db.is_banned(user_id):
...
# ---------------------------------------------------
# /users Command (Standalone + JSON export, Cached)
# ---------------------------------------------------
...
async def users_count(bot: Client, message: Message):
    ...
    try:
        ...
        await msg.edit_text(
            f"""
🌀 <b><i>User Analytics Update V2</i></b> 🌀

👥 <b>Total Registered Users:</b> {total}
🛰 <b>System Status:</b> Active ✅
🧠 <b>Data Source:</b> MongoDB (cached)
DEVs: 1. @DmOwner 2. @akaza7902
"""
        )
